...
const AddCustomerModal = ({ onClose }) => {
  ...
  useEffect(()=>{
    const fetchCustemr=async()=>{
      try {
        const res=await axiosInstance.get(`/admin/customer/get/${id}`)
        console.log(res)
setFormData(prv=>({ 
   ...prv,
  ...
  discountapplied:res.data.customer.discountApplied,
  route:res.data.customer.routeCustomer,
  discount:res.data.customer.discount,
  balance:res.data.customer.openingBalance,
}))
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axiosInstance.put(`/admin/customer/edit/${id}`, payload);
