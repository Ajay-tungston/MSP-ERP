...
async function generateMonthlyClosure() {
  const now = new Date();
  const monthStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`; // "2025-05"
  ...
  const paymentsIn = await Payment.aggregate([
    { $match: { paymentType: "PaymentIn", date: { $gte: startOfMonth, $lte: endOfMonth } } },
    { $group: { _id: null, total: { $sum: "$amount" } } },
  ]);

  const paymentsOut = await Payment.aggregate([
    { $match: { paymentType: "PaymentOut", date: { $gte: startOfMonth, $lte: endOfMonth } } },
    { $group: { _id: null, total: { $sum: "$amount" } } },
  ]);
